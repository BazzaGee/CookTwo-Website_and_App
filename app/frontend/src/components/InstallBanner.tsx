...
export function InstallBanner() {
  const { isInstallable, showIOSHint, install, dismiss, dismissIOS } = useInstallPrompt();
  ...
  if (showIOSHint) {
    return (
      <div className="fixed top-0 inset-x-0 z-50 flex justify-center bg-cream border-b border-border shadow-sm animate-slide-up">
        <div className="w-full max-w-md px-6 py-4 flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <div className="w-8 h-8 bg-sage/10 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
              <Download size={16} className="text-sage" />
            </div>
            <div>
              <p className="text-text-primary text-sm font-medium">Install CookTwo on your iPhone</p>
              <p className="text-text-secondary text-xs leading-relaxed mt-1">
                Tap <Share size={11} className="inline -mt-0.5 mx-0.5" /> Share in Safari, then{' '}
          ...
          <button
            type="button"
            onClick={dismissIOS}
            className="p-1.5 rounded-full text-text-secondary hover:text-text-primary transition-colors flex-shrink-0"
            aria-label="Dismiss"
          >
            <X size={16} />
          </button>
